...
function Cart() {
    ...
    if (savedItems) {
      const itemsArray = savedItems.split(",").map((item) => {
        ...
        return {
          Item: itemProperties[0],
          Name: itemProperties[1],
          MRP: parseFloat(itemProperties[2]),
          caption: itemProperties[3],
          img: itemProperties[4],
          quantity: parseInt(itemProperties[5] || 1), // Handle both old and new format
        };
      });
      setCartItems(itemsArray);
    }
  ...
  const updateLocalStorage = (updatedCartItems) => {
    const updatedCartItemsString = updatedCartItems
      .map(
        (item) =>
          `${item.Item}|${item.Name}|${item.MRP}|${item.caption}|${item.img}|${item.quantity}`
      )
    ...
    <div id="CartMain">
      <div id="CartUpper">
        {cartItems.length === 0 ? (
          <div id="CartUpper1">
            <h2>
              "Your cart is feeling lonely! 🛒💔 Start adding your favorites
              now! 😊"
            </h2>
            <div id="CartUpperImg">
              <img src="/EmptyBasket.png" alt="" />
            ...
            <div>
              {" "}
              <button onClick={goproduct}>Explore Product</button>
            </div>
          </div>
        ) : (
          <div id="Superclass">
